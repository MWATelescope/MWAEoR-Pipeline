...
def get_parser():
    ...
    tec_group.add_argument('--radec', default=[], nargs=2, type=float,
                           help="ra/dec for centre of TEC plot")
    tec_group.add_argument('--altaz', default=[], nargs=2, type=float,
    ...
    tec_group.add_argument('--frequency', default=200e6, type=int,
...
def main():
    ...
    wavelength = 299792458 / args.frequency
    ...
    if ext == ".json":
        ...
        l_shifts = alphas * wavelength_2
        m_shifts = betas * wavelength_2
    ...
    if args.altaz:
        centroid_radec = SkyCoord(az=args.altaz[1], alt=args.altaz[0], unit=(u.deg, u.deg), frame="altaz",
                    obstime=centroid_time, location=MWA_LOCATION).transform_to(ICRS)
        # aaframe = AltAz(location=MWA_LOCATION, obstime=centroid_time)
        # centroid_radec = SkyCoord(radians(args.altaz[1]), radians(args.altaz[0]), unit="rad", frame=aaframe).transform_to(ICRS)
        centroid_ra, centroid_dec = centroid_radec.ra.deg, centroid_radec.dec.deg
    elif args.radec:
        centroid_ra, centroid_dec = args.radec
    else:
    ...
    if ra_shifts is None or dec_shifts is None:
        ...
        for i in range(n_times):
            ra_shifts_ts, dec_shifts_ts = lm2radec(ras, decs, np.rad2deg(l_shifts[:, i]), np.rad2deg(m_shifts[:, i]), {"primary_beam_pointing_centre": [centroid_ra, centroid_dec]})
    ...
    o.ra_centre = centroid_ra
    o.dec_centre = centroid_dec
    o.pca()
    o.obsid_metric()
    ...
    if args.plot or args.tec:
        ...
        if args.average:
            ...
            o.reconstruct_tec(resolution=args.resolution, frequency=args.frequency/1e6, interp_method=args.interpolation)
            ...
            for t in range(n_times):
                ...
                if args.altaz:
                    ...
                    radec = SkyCoord(radians(args.altaz[1]), radians(args.altaz[0]), unit="rad", frame=aaframe).transform_to(ICRS)
                ...
                print(f"{len(sane_idxs)=} {len(t_total_shifts)=}")
                ...
                o.obsid_metric()
                # o.reconstruct_tec(resolution=args.resolution, frequency=args.frequency/1e6, interp_method=args.interpolation)
                o.reconstruct_tec(frequency=args.frequency/1e6, interp_method=args.interpolation)
                o.tec_power_spectrum()
                o.metrics.append(
                ...
                grid_dra = griddata(pixels[sane_idxs], t_ra_shifts[sane_idxs], (grid_x, grid_y), method=args.interpolation, fill_value=0)
                grid_ddec = griddata(pixels[sane_idxs], t_dec_shifts[sane_idxs], (grid_x, grid_y), method=args.interpolation, fill_value=0)
                if args.blur:
                    grid_dra = signal.fftconvolve(grid_dra, kernel, mode='same')
                    grid_ddec = signal.fftconvolve(grid_ddec, kernel, mode='same')
                tec_grid = g2s(x_axis, y_axis, grid_dra, grid_ddec)
                tec_grid -= np.min(tec_grid)
                ...
                if args.offset_type == "scatter":
                    ...
                    cmap = plt.cm.hsv
                    # ax.quiver(pixels_x[plot_idxs], pixels_y[plot_idxs], t_ra_shifts[plot_idxs], t_dec_shifts[plot_idxs],
                    #           angles='xy', scale_units='xy', scale=args.resolution / 10, color=cmap(angles[plot_idxs]))
                    ax.quiver(o.fra, o.fdec, o.fra_shifts, o.fdec_shifts, transform=ax.get_transform('world'), color=cmap(angles))
