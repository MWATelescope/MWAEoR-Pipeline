...
def plot_delay_spec(delays, bin_edges, all_delay_spec_by_bin, name, title=None,
    ...
    add_colourbar(ax=ax, im=im, fig=fig, label='log10 P(b) mK\$^2\$ \$h^{-3}\$ Mpc\$^3\$')
...
def plot_delay_spec_multi_uvfits(args):
    ...
    plot_delay_spec(delays, bin_edges, all_delay_spec_by_bin, name, args.plot_title,
                    args.vmin, args.vmax)
...
def get_parser():
    ...
    plot_group.add_argument('--vmax', default=False, type=float,
        help='Maximum value for colourscale (e.g. 1e+16)')
    plot_group.add_argument('--vmin', default=False, type=float,
...
def main():
    ...
    singularity exec /pawsey/mwa/singularity/ssins/ssins_latest.sif python \
        /astro/mwaeor/dev/MWAEoR-Pipeline/templates/jline_delay_spec_from_uvfits.py \
        --uvfits=/astro/mwaeor/dev/nfdata/1061315448/cal/hyp_1061315448_sub_30l_src4k_8s_80kHz.uvfits \
        --output_name=/astro/mwaeor/dev/nfdata/1061315448/vis_qa/dlyspec_1061315448_sub_30l_src4k_8s_80kHz.png \
